                                ...
                                <div class="p-5">
                                    ...
                                    @if($value->options->in_stock < $value->qty)
                                        <div class="alert alert-danger">
                                            Product in stock is not enough!! Please order less or pick another
                                        </div>
                                    @endif
                                    <div class="row mb-4 d-flex justify-content-between align-items-center 
                                        {{$value->options->in_stock < $value->qty ? 'bg-danger' : ''}}">
                                        <div class="col-md-2 col-lg-2 col-xl-2">
                                            <img src="{{URL::to('/public/upload/product/'.$value->options->image)}}" 
                                        ...
                                        <div class="col-md-3 col-lg-3 col-xl-2">
                                            <form action="{{URL::to('/update-cart')}}" method="POST">
                                                @csrf
                                                <input min="1" name="cart_quantity" value="{{$value->qty}}"
                                                     type="number" class="form-control form-control-sm" />
                                                <input type="hidden" value="{{$value->rowId}}" name="rowId_cart">
                                                <button type="submit" value="Update" name="update_qty" 
                            ...
                            <div class="col-lg-4 bg-grey">
                                <div class="p-5">
                                    ...
                                    <form class="card p-2" method="POST" action="{{URL::to('/check-coupon')}}">
                                        ...
                                        <div class="input-group">
                                            ...
                                            @if(Session::get('coupon') == null)
                                                <button type="submit" class="btn btn-danger check_coupon" 
                                                    name="check_coupon">
                                                    Add code
                                                </button>
                                            @else
                                                <button disabled type="submit" class="btn btn-dark check_coupon" 
                                                    name="check_coupon">
                                                    Add code
                                                </button>
                                            @endif
                                    ...
                                    @if(Session::get('account_id') != null)
                                        <a href="{{URL::to('/check-out')}}" type="button" 
                                            class="btn btn-dark btn-block btn-lg" data-mdb-ripple-color="dark">
                                            Checkout
                                        </a>
                                    @else
                                        <a href="{{URL::to('/login')}}" type="button" 
                                            class="btn btn-dark btn-block btn-lg" data-mdb-ripple-color="dark">
                                            Checkout
                                        </a>
                                    @endif
                                    @if(Session::get('coupon') != null)
                                        <a style="margin-left: 50px;" href="{{URL::to('/unset-coupon')}}" 
                                            type="button" class="btn btn-dark btn-block btn-lg" data-mdb-ripple-color="dark">
                                            Unset coupon
                                        </a>
                                    @endif
                                </div>
